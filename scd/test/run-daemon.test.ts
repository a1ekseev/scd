...
import { getErrorLogDetails, runCronLoop, runLoggedCronLoop } from '../src/runtime/run-daemon.ts';
...
test('runCronLoop skips overrunning ticks without drifting future cron slots', async () => {
  let nowMs = 0;
  let stopping = false;
...
test('runLoggedCronLoop logs overrun through logger path', async () => {
  let nowMs = 0;
  let stopping = false;
  let runCount = 0;
  let releaseFirstRun!: () => void;
...
test('runLoggedCronLoop logs aggregate causes through logger path', async () => {
  let stopping = false;
  const warnings: CapturedLog[] = [];
  const errors: CapturedLog[] = [];
  ...
  await runLoggedCronLoop(
    ...
    async () => {
      stopping = true;
      throw new AggregateError([new Error('first cause'), new Error('second cause')], 'tick failed');
    },
    {
      warn(...args: unknown[]) {
        captureLog(warnings, ...args);
  ...
  assert.equal(errors[0]?.message, 'Monitor tick failed.');
  assert.equal(errors[0]?.object.event, 'monitor_tick_failed');
  assert.deepEqual(errors[0]?.object.causes, ['first cause', 'second cause']);
});
