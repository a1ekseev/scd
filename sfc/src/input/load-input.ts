...
import type { LoadInputOptions, LoadedInput, SubscriptionInputFormat } from '../types.ts';
...
function applyInputFormat(raw: string, format: SubscriptionInputFormat = 'auto'): LoadedInput {
  if (format === 'plain') {
    return { source: '', content: raw, encoding: 'plain' };
  }

  if (format === 'base64') {
    return {
      source: '',
      content: Buffer.from(normalizeBase64(raw), 'base64').toString('utf8'),
      encoding: 'base64',
    };
  }
