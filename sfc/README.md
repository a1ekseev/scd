...
- `subscriptions[].outputs[].id`: globally unique opaque public id served at `/s/<id>`.
...
- `GET /s/<output-id>`: returns the current cached filtered subscription payload as plain base64 text.

Response semantics for `GET /s/<output-id>`:
